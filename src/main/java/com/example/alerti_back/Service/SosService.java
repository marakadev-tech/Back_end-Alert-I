...
public class SosService {
    ...
    private void envoyerNotificationsUrgence(SosSignal sosSignal) {
        try {
            // Notification à l'utilisateur qui a signalé (si non anonyme)
            if (!sosSignal.isAnonyme() && sosSignal.getUserId() > 0) {
                boolean confirmationSent = notificationService.sendSosConfirmationToUser(sosSignal, sosSignal.getUserId());
                if (confirmationSent) {
                    System.out.println("📱 Notification de confirmation SOS envoyée à l'utilisateur " + sosSignal.getUserId());
                }
            }
            
            // Notification générale à tous les utilisateurs
            boolean generalNotificationSent = notificationService.sendSosNotification(sosSignal, "new_signal");
            if (generalNotificationSent) {
                System.out.println("📢 Notification SOS générale envoyée à tous les utilisateurs");
            }
            
            // Notification critique aux autorités
            if ("critique".equals(sosSignal.getPriorite())) {
                boolean criticalAlertSent = notificationService.sendCriticalSosAlert(sosSignal);
                if (criticalAlertSent) {
                    System.out.println("🚨 ALERTE CRITIQUE - Notification d'urgence critique envoyée!");
                }
            }
            
        } catch (Exception e) {
            System.err.println("❌ Erreur envoi notifications urgence: " + e.getMessage());
        }
    }
